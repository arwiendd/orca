    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    m_link_edit_nozzle->SetForegroundColour("#f6cc1d");
    ...
    m_statictext_finish->SetForegroundColour(wxColour(246, 204, 29));
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#f6cc1d") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
