...
*/
    ...
    case GUI_App::EAppMode::Editor:
        ...
        m_taskbar_icon->SetIcon(wxIcon(Slic3r::var("OrcaSlicer-mac_256px.ico"), wxBITMAP_TYPE_ICO), "Confabric Slicer");
        break;
...
void MainFrame::update_side_button_style()
{
    ...
    StateColor m_btn_bg_enable = StateColor(
        std::pair<wxColour, int>(wxColour(212, 173, 10), StateColor::Pressed),
...
void MainFrame::load_config_file()
{
 ...
 //       return;
    // Confabric: Do not allow importing printer profiles (.orca_printer files removed from filter)
    wxFileDialog dlg(this, _L("Select profile to load:"),
        !m_last_config.IsEmpty() ? get_dir_name(m_last_config) : wxGetApp().app_config->get_last_dir(),
        "config.json", "Config files (*.json;*.zip;*.orca_filament)|*.json;*.zip;*.orca_filament", wxFD_OPEN | wxFD_MULTIPLE | wxFD_FILE_MUST_EXIST);
     wxArrayString files;
    if (dlg.ShowModal() != wxID_OK)
        return;
    dlg.GetPaths(files);
